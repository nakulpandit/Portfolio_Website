...
void main() {
  ...
  float streaks = pow(abs(sin(angle * 46.0 + uTime * 11.0)), 6.0);
  ...
  float haze = smoothstep(1.1, 0.3, radius);

  vec3 cyan = vec3(0.35, 0.72, 1.0);
  vec3 warm = vec3(1.0, 0.93, 0.82);
  vec3 color = mix(cyan, warm, core * 0.85 + rings * 0.1);
  float alpha = (streaks * tunnel * 0.9 + core * 1.2 + sparks * edgeGlow * 0.28 + haze * 0.2) * uIntensity;
